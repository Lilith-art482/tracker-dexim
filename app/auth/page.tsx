        ...
        <div className="lg:hidden flex flex-col w-full">
          ...
          <div className="flex items-center justify-between px-4 pt-4 pb-2">
            <div className="flex items-center gap-2">
              <Image src="/logo.png" alt="In Motion" width={32} height={32} className="h-7 w-auto" priority />
              <span className="text-lg font-bold text-foreground">In Motion</span>
            </div>
            <button
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  {!isLogin && (
                    <div className="space-y-2 animate-in fade-in slide-in-from-top-4 duration-300">
                      <label className="text-sm font-medium text-foreground" htmlFor="m-nickname">Ваш никнейм или имя</label>
                      <div className="relative">
                        <User className="absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 text-muted-foreground" />
                        <input id="m-nickname" type="text" value={formData.nickname}
                  ...
                  <div className="space-y-2">
                    <label className="text-sm font-medium text-foreground" htmlFor="m-email">Email</label>
                    <div className="relative">
                      <Mail className="absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 text-muted-foreground" />
                      <input id="m-email" type="email" value={formData.email}
                  ...
                  <div className="space-y-2">
                    <label className="text-sm font-medium text-foreground" htmlFor="m-password">Пароль</label>
                    <div className="relative">
                      <Lock className="absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 text-muted-foreground" />
                      <input id="m-password" type="password" value={formData.password}
                  ...
                  <button type="submit" disabled={loading}
                    ...
                    <span className="relative z-10 flex items-center justify-center gap-2">
                      {loading ? (<><Loader2 className="h-5 w-5 animate-spin" /><span>{isLogin ? "Входим..." : "Регистрация..."}</span></>) : isLogin ? "Войти" : "Зарегистрироваться"}
                    </span>
                  </button>
                </form>
              </div>
          ...
          <div className="px-4 pb-8 space-y-4">
            <p className="text-center text-sm text-muted-foreground font-medium">Возможности</p>
            <div className="grid grid-cols-2 gap-3">
              {features.map((f) => (
                <div key={f.title} className="rounded-xl bg-card/50 border border-border/40 p-3 space-y-2">
          ...
          <div className="w-1/2 flex flex-col justify-center px-12 xl:px-20 py-12">
            <div className="max-w-lg mx-auto w-full">
              <div className="flex items-center gap-3 mb-6">
                <Image src="/logo.png" alt="In Motion" width={40} height={40} className="h-9 w-auto" priority />
                <span className="text-2xl xl:text-3xl font-bold text-foreground leading-none">In Motion</span>
              </div>
              <p className="text-lg xl:text-xl text-muted-foreground mb-10 leading-relaxed">
                Единое пространство для твоих задач, финансов и привычек — всё, чтобы оставаться на пути к целям.
              </p>
                ...
                {features.map((f) => (
                  <div key={f.title} className="flex items-start gap-4 group">
                    ...
                    <div className="pt-0.5">
                      <h3 className="text-foreground font-semibold mb-1 text-base">{f.title}</h3>
            ...
            <div className="relative w-full max-w-md">
              ...
              <div className="backdrop-blur-2xl bg-card/70 border border-border/60 rounded-3xl shadow-2xl p-8 animate-in fade-in slide-in-from-bottom-8 duration-700">
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                  {!isLogin && (
                    <div className="space-y-2 animate-in fade-in slide-in-from-top-4 duration-300">
                      <label className="text-sm font-medium text-foreground" htmlFor="d-nickname">Ваш никнейм или имя</label>
                      <div className="relative">
                        <User className="absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 text-muted-foreground" />
                        <input id="d-nickname" type="text" value={formData.nickname}
                          onChange={(e) => setFormData({ ...formData, nickname: e.target.value })}
                          className="w-full pl-10 pr-4 py-3 bg-muted/30 border border-border/60 rounded-xl text-foreground placeholder:text-muted-foreground/50 focus:outline-none focus:ring-2 focus:ring-primary/50 focus:border-transparent transition-all"
                          placeholder="Как вас зовут?" required minLength={2} maxLength={30} />
                      </div>
                  ...
                  <div className="space-y-2">
                    <label className="text-sm font-medium text-foreground" htmlFor="d-email">Email</label>
                    <div className="relative">
                      <Mail className="absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 text-muted-foreground" />
                      <input id="d-email" type="email" value={formData.email}
                        onChange={(e) => setFormData({ ...formData, email: e.target.value })}
                        className="w-full pl-10 pr-4 py-3 bg-muted/30 border border-border/60 rounded-xl text-foreground placeholder:text-muted-foreground/50 focus:outline-none focus:ring-2 focus:ring-primary/50 focus:border-transparent transition-all"
                        placeholder="your@email.com" required />
                    </div>
                  ...
                  <div className="space-y-2">
                    <label className="text-sm font-medium text-foreground" htmlFor="d-password">Пароль</label>
                    <div className="relative">
                      <Lock className="absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 text-muted-foreground" />
                      <input id="d-password" type="password" value={formData.password}
                        onChange={(e) => setFormData({ ...formData, password: e.target.value })}
                        className="w-full pl-10 pr-4 py-3 bg-muted/30 border border-border/60 rounded-xl text-foreground placeholder:text-muted-foreground/50 focus:outline-none focus:ring-2 focus:ring-primary/50 focus:border-transparent transition-all"
                        placeholder="••••••••" required minLength={6} />
                    </div>
