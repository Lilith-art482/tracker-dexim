...
interface HabitContextValue {
  ...
  toggleHabit: (habitId: string, status: "done" | "missed" | "skipped") => Promise<void>;
  toggleHabitForDate: (habitId: string, date: string, status: "done" | "missed" | "skipped") => Promise<void>;
  addReminder: (data: { habitId: string; time: string; daysOfWeek?: number[] }) => Promise<void>;
...
export function HabitProvider({ children }: { children: React.ReactNode }) {
  ...
  const isScheduledToday = useCallback((habit: Habit): boolean => {
    ...
    if (habit.frequencyType === "interval" && habit.frequencyInterval) {
      ...
      const daysSince = Math.floor(
        (new Date(today + "T00:00:00Z").getTime() - new Date(lastLog.date + "T00:00:00Z").getTime()) / 86400000,
      );
  ...
  const stats = useMemo((): HabitStats => {
    ...
    const allCompletion = active.map((h) => calculateCompletionPercentage(h.id, logs, 30));
    const avgCompletion = active.length > 0 ? Math.round(allCompletion.reduce((s, v) => s + v, 0) / active.length) : 0;
    return {
      total: active.length,
      doneToday,
  ...
  const addHabit = useCallback(async (data: Partial<Habit>): Promise<Habit | null> => {
    try {
  ...
  const cloneHabit = useCallback(async (id: string) => {
  ...
  const toggleHabit = useCallback(async (habitId: string, status: "done" | "missed" | "skipped") => {
    try {
      ...
      setLogs((prev) => {
        const filtered = prev.filter((l) => !(l.habitId === habitId && l.date === today));
  ...
  const toggleHabitForDate = useCallback(async (habitId: string, date: string, status: "done" | "missed" | "skipped") => {
    try {
      const res = await fetch("/api/habit-logs", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const addReminder = useCallback(async (data: { habitId: string; time: string; daysOfWeek?: number[] }) => {
    try {
  ...
  const value = useMemo(
    ...
    [habits, logs, achievements, reminders, todayHabits, stats, loading, addHabit, updateHabit, deleteHabit, cloneHabit, toggleHabit, toggleHabitForDate, addReminder, updateReminder, deleteReminder, addAchievement, fetchAll],
  );
